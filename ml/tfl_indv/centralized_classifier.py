...
df = []
for i in csvs:
    for j in i: df.append(pd.read_csv(j))
df = pd.concat(df)
...
data.to_csv("data/preprocessed/X_all.csv", index=False, header=None)
labels.to_csv("data/preprocessed/y_all.csv", index=False, header=None)

"""

data = pd.read_csv("data/preprocessed/X_all.csv").as_matrix()
labels = pd.read_csv("data/preprocessed/y_all.csv").as_matrix()
...
checkpoint = ModelCheckpoint("models/centralized_model.hdf5", monitor='val_acc', verbose=1, save_weights_only=False, save_best_only=True)
csv_logger = CSVLogger("models/centralized_history.csv", separator=',', append=False)
...
model = load_model("models/centralized_model.hdf5")
...
plt.savefig("outputs/centralized_cm.png", dpi=200, format='png', bbox_inches='tight', pad_inches=0.5); plt.close();

plot_auroc(y_pred, y_test, classes, title = 'Area Under the Reciever Operating Characteristics')
plt.savefig("outputs/centralized_auroc.png", dpi=200, format='png', bbox_inches='tight', pad_inches=0.5); plt.close();

plot_clr(predictions, actuals, classes, cmap='RdBu', figsz=(30,15), title = 'Classification Report')
plt.savefig("outputs/centralized_clr.png", dpi=200, format='png', bbox_inches='tight', pad_inches=0.25); plt.close();
...
fig.add_subplot(221); plt.imshow(plt.imread("outputs/centralized_cm.png")); plt.axis('off'); os.remove("outputs/centralized_cm.png")
fig.add_subplot(222); plt.imshow(plt.imread("outputs/centralized_auroc.png")); plt.axis('off'); os.remove("outputs/centralized_auroc.png")
fig.add_subplot(212); plt.imshow(plt.imread("outputs/centralized_clr.png")); plt.axis('off'); os.remove("outputs/centralized_clr.png")

plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)

plt.savefig("outputs/centralized_output_derivations.png", dpi=700, format='png'); plt.close();